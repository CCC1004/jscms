...
strFrame+='    </table></td></tr>';
strFrame+='    <tr ><td >';
strFrame+='        <table border=0 cellspacing=1 cellpadding=0 width=100%  bgcolor=#FFFFFF height=22 id="TimeTable">';
...
function setdayDT(tt,obj) //主调函数,显示时间选择
{
        bUseTime=true;
...
function setdayD(tt,obj) //主调函数,只有日期选择
{
        bUseTime=false;
        if (arguments.length == 2)
        	setday(tt,obj);
...
function setday(tt,obj) //主调函数
{
    ...
    if(r!=null){
        ...
        if(d.getFullYear()==r[1] && d.getMonth()==r[3] && d.getDate()==r[4])
        {
            outDate=d;
            meizzTheYear = parseInt(r[1]);
            meizzTheMonth = parseInt(r[3]);
            meizzTheDate = parseInt(r[4]);
            if(bUseTime)
            {
            	meizzTheHour = parseInt(r[5]);
            	meizzTheMinute = parseInt(r[6]);
            	meizzTheSecond = parseInt(r[7]);            	            	
            }
        }
        else
        {
        ...
        meizzSetDay(r[1], parseInt(r[3])+1);
    }
    else
    {
        outDate="";
	...
	if(bUseTime)
	{
         window.frames.endDateLayer.document.getElementById("TimeTable").style.display ="";			
	}
	else
	{
        window.frames.endDateLayer.document.getElementById("TimeTable").style.display ="none";				
	}
...
function meizzToday()    //Today Button
{
    ...
    if(outObject)
    {
        if (bUseTime)
        {
            outObject.value= meizzTheYear + "-" + format( meizzTheMonth) + "-" + 
                            format(meizzTheDate) + " " + format(meizzTheHour) + ":" + 
        ...
        {
            outObject.value= meizzTheYear + "-" + format( meizzTheMonth) + "-" + 
                            format(meizzTheDate); //注：在这里你可以输出改成你想要的格式
        }
    }
    closeLayer();
}
