...
Ext.extend(Ext.ux.UploadDialog.Dialog, Ext.Window, {
  ...
  createGrid : function()
  {
  	var dataProxy=new Ext.data.HttpProxy({url:"fileListByGroup.qact?fileGroup="+this.groupId});   
    ...
    */
    var cm = new Ext.grid.ColumnModel([
      ...
      },
      {
        ...
        dataIndex: 'note',
        sortable: true,
        renderer: function(val, cellmeta, record, rowIndex, colIndex, store){
      ...
      ,
      {
        ...
        renderer: function(val, cellmeta, record, rowIndex, colIndex, store){
        	//var cmHead;
		    //cmHead=this.grid_panel.getColumnModel().getColumnHeader(colIndex);
		    if(val == ''){
		    	return '';
		    }
		    val= '<a href="'+'download.qact?docId='+val+'">'+'下载'+'</a>';
		    return val;
        }
      }
    ]);
    ...
    store.each(function(r) {
      if (!record && r.get('state') == Ext.ux.UploadDialog.FileRecord.STATE_QUEUE) {
        record = r;
      	...
      	if(r.get('input_element') != null && r.get('input_element') != ""){
  ...
  resetQueue : function()
  {
    ...
    store.each(
      function(r) {
      	if(r.get('input_element') != '' && r.get('state') != '1'){
      		r.get('input_element').remove();
      		store.remove(r);
      	}
