...
Ext.ux.maximgb.treegrid.AbstractTreeStore = Ext.extend(Ext.data.Store,
{
	...
	load : function(options)
	{
		...
		if (options.params[this.paramNames.active_node] !== null) {
			//Modify by Lo
			//Last: options.add = true;
			options.add = this.is_tree;
...
Ext.ux.maximgb.treegrid.AdjacencyListStore = Ext.extend(Ext.ux.maximgb.treegrid.AbstractTreeStore,
{
	 ...
	 */
	parent_id_field_name : '_parent_pk',
	id_field_name: '_table_pk',
	...
	getRootNodes : function()
	{
		...
		for (i = 0, len = records.length; i < len; i++) {
			//add By Lo || Ext.util.Format.trim(records[i].get(this.parent_id_field_name)) ==""
			if (this.rootVal=='' && (records[i].get(this.parent_id_field_name) == null 
				|| Ext.util.Format.trim(records[i].get(this.parent_id_field_name)) =="" ) ) {
				result.push(records[i]);
			}
			else if(Ext.util.Format.trim(records[i].get(this.id_field_name)) ==this.rootVal )
				result.push(records[i]);
		}
...
Ext.ux.maximgb.treegrid.GridView = Ext.extend(Ext.grid.GridView, 
{
					...
					'<div class="x-grid3-header">',
						...
						'<div class="x-grid3-header-inner">',
							'<div class="x-grid3-header-offset">',
								'<div class="ux-maximgb-treegrid-breadcrumbs" unselectable="on">&#160;</div>',
							'</div>',
						'</div>',
    ...
    ts.cell = new Ext.Template(
		...
		ts.brd_item = new Ext.Template(
			'<a id="I{id}" class="ux-maximgb-treegrid-brditem" title="{title}" unselectable="on">{caption}</a>'
		);
  ...
  initElements : function()
  {
	...
	doRender : function(cs, rs, ds, startRow, colCount, stripe)
	{
        ...
        p.style = c.style;
				...
				// ----- Modification start
				if (c.id == this.grid.master_column_id && ds.is_tree) {
  ...
  renderCellTreeUI : function(record, store)
  {
	...
	{
		//if(this.grid.master_column_id == -1)
		//Add by Lo
		if(!store.is_tree)
			return;
...
Ext.ux.maximgb.treegrid.GridPanel = Ext.extend(Ext.grid.GridPanel, 
{
	...
	initComponent : function()
	{
		...
		//add by Lo
		//if(!this.getStore().is_tree)
		//	master_column_id=null;
