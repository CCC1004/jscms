...
///下面的验证类型名称作了很大的修改,另修改一处 getElmID()函数--柯
Validator.messageSource['en-us'] = [
	['validation-failed' , 'Validation failed.'],
	['required' , 'This is a required field.'],
	['double' , 'Please enter a valid number in this field.'],
	['digit' , 'Please use numbers only in this field. please avoid spaces or other characters such as dots or commas.'],
	['letter' , 'Please use letters only (a-z) in this field.'],
	['letter-number' , 'Please use only letters (a-z) or numbers (0-9) only in this field. No spaces or other characters are allowed.'],
	['email' , 'Please enter a valid email address. For example fred@domain.com .'],
	['url' , 'Please enter a valid URL.'],
	['currency-dollar' , 'Please enter a valid $ amount. For example $100.00 .'],
	['one-required' , 'Please select one of the above options.'],
	['int' , 'Please enter a valid integer in this field'],
	['pattern' , 'Validation failed.'],
	['ip','Please enter a valid IP address'],
	['value-min' , 'min value is %s.'],
	['value-max' , 'max value is %s.'],
	['length-min' , 'min length is %s,current length is %s.'],
	['length-max' , 'max length is %s,current length is %s.'],
	['int-range' , 'Please enter integer value between %s and %s'],
	['double-range' , 'Please enter number between %s and %s'],
	['length-range' , 'Please enter value length between %s and %s,current length is %s'],
	['length-equal' , 'Please enter value length %s,current length is %s'],	
	['field-equals','Conflicting with above value.'],
	['field-min','Input value must be less than above value.'],
	['field-min-equal','Input value must be less than or equal above value.'],
	['field-max','Input value must be great than above value.'],
	['field-max-equal','Input value must be great than or equal above value.'],
	['date' , 'Please use this date format: %s. For example %s.'],
	['one-selection' , 'Please make a selection.'],
	['file' , function(v,elm,args,metadata) {
		return ValidationUtils.format("Please enter file type in [%s]",[args.join(',')]);
	}],
	//中国特有的相关验证提示信息
	['id-number','Please enter a valid id number.'],
	['chinese','Please enter chinese'],
	['phone','Please enter a valid phone number,current length is %s.'],
	['mobile-phone','Please enter a valid mobile phone,For example 13910001000.current length is %s.'],
	['zip','Please enter a valid zip code.'],
	['qq','Please enter a valid qq number']
]
...
Validator.messageSource['zh-cn'] = [
	...
	['digit' , '请输入数字.'],
	['letter' , '请输入英文字母.'],
	['letter-number' , '请输入英文字母或是数字,其它字符是不允许的.'],
	['email' , '请输入有效的邮件地址,如 username@example.com.'],
	['url' , '请输入有效的URL地址.'],
	['currency-dollar' , 'Please enter a valid $ amount. For example $100.00 .'],
	['one-required' , '在前面选项至少选择一个.'],
	['int' , '请输入正确的整数'],
	['pattern' , '输入的值不匹配'],
	['ip','请输入正确的IP地址'],
	['value-min' , '最小值为%s'],
	['value-max' , '最大值为%s'],
	['length-min' , '最小长度为%s,当前长度为%s.'],
	['length-max', '最大长度为%s,当前长度为%s.'],
	['int-range' , '输入值应该为 %s 至 %s 的整数'],
	['double-range' , '输入值应该为 %s 至 %s 的数字'],
	['length-range' , '输入值的长度应该在 %s 至 %s 之间,当前长度为%s'],
	['length-equal' , '输入值的长度应该为 %s,当前长度为%s'],	
	['field-equals','两次输入不一致,请重新输入'],
	['field-min','请输入小于前面的值'],
	['field-min-equal','请输入小于或等于前面的值'],
	['field-max','请输入大于前面的值'],
	['field-max-equal','请输入大于或等于前面的值'],
	['date' , '请输入有效的日期,格式为 %s. 例如:%s.'],
	['one-selection' , '请选择.'],
	['file' , function(v,elm,args,metadata) {
		return ValidationUtils.format("文件类型应该为[%s]其中之一",[args.join(',')]);
	}],
	//中国特有的相关验证提示信息
	['id-number','请输入合法的身份证号码'],
	['chinese','请输入中文'],
	['phone','请输入正确的电话号码,如:010-29392929,当前长度为%s.'],
	['mobile-phone','请输入正确的手机号码,当前长度为%s.'],
	['zip','请输入有效的邮政编码'],
	['qq','请输入有效的QQ号码.']
]
...
ValidationUtils = {
	...
	getElmID : function(elm) {
		...
		return elm.name ? elm.name : elm.id;
	},
	...
	// 返回值包含一个参数singleArgument,例:pattern-/[a-c]/gi,singleArgument值为/[a-c]/gi
...
Object.extend(Validation, {
	...
	newErrorMsgAdvice : function(name,elm,errorMsg) {
		var advice = '<br><div class="validation-advice" id="advice-' + name + '-' + ValidationUtils.getElmID(elm) +'" style="display:none">' + errorMsg + '</div>'
...
Validation.addAllThese([
	...
	['letter-number', function(v) {
				return !/\W/.test(v)
			}],
	['one-required', function (v,elm) {
				var p = elm.parentNode;
...
Validation.addAllThese([
	/**
	 * Usage : field-equals-$otherInputId
	 * Example : field-equals-username or field-equals-email etc..
	 */
	['field-equals', function(v,elm,args,metadata) {
				return $F(args[0]) == v;
			},{ignoreEmptyValue:false}],
	/**
	 * Usage : field-min-$otherInputId
	 */
	['field-min', function(v,elm,args,metadata) {
				if(Validation.get('double').test(v) && Validation.get('validate-number').test($F(args[0])))
					return parseFloat(v) < parseFloat($F(args[0]));
				return v < $F(args[0]);
			}],
	/**
	 * Usage : field-min-equal-$otherInputId
	 */
	['field-min-equal', function(v,elm,args,metadata) {
				if(Validation.get('double').test(v) && Validation.get('double').test($F(args[0])))
					return parseFloat(v) <= parseFloat($F(args[0]));
				return v < $F(args[0]) || v == $F(args[0]);
			}],			
	/**
	 * Usage : field-max-$otherInputId
	 */
	['field-max', function(v,elm,args,metadata) {
				if(Validation.get('double').test(v) && Validation.get('double').test($F(args[0])))
					return parseFloat(v) > parseFloat($F(args[0]));
				return v > $F(args[0]);
			}],
	/**
	 * Usage : field-max-equal-$otherInputId
	 */
	['field-max-equal', function(v,elm,args,metadata) {
				if(Validation.get('double').test(v) && Validation.get('double').test($F(args[0])))
					return parseFloat(v) >= parseFloat($F(args[0]));
				return v > $F(args[0]) || v == $F(args[0]);
			}],			
	/*
	 * Usage: length-min-$number
	 * Example: length-min-10
	 */
	['length-min',function(v,elm,args,metadata) {
		return v.length >= parseInt(args[0]);
	}],
	/*
	 * Usage: length-max-$number
	 * Example: length-max-10
	 */
	['length-max',function(v,elm,args,metadata) {
		return v.length <= parseInt(args[0]);
	}],
	/*
	...
	/*
	 * Usage: double-range-$minValue-$maxValue
	 * Example: -2.1 to 3 = double-range--2.1-3
	 */
	['double-range',function(v,elm,args,metadata) {
	...
	/*
	 * Usage: length-equal-$length
	 * Example: 20 = length-equal-20
	 */
	['length-equal',function(v,elm,args,metadata) {
		return (v.length == parseInt(args[0]))
	}],	
	/*
	 * Usage: value-max-$number
	 * Example: value-max-10
	 */
	['value-max',function(v,elm,args,metadata) {
		return parseFloat(v) <= parseFloat(args[0]);
	},{depends : ['double']}],
	/*
	 * Usage: value-min-$number
	 * Example: value-min-10
	 */
	['value-min',function(v,elm,args,metadata) {
		return parseFloat(v) >= parseFloat(args[0]);
	},{depends : ['double']}],
	/*
	 * Usage: pattern-$RegExp
	 * Example: <input id='sex' class='pattern-/^[fm]$/i'>
	 */
	['pattern',function(v,elm,args,metadata) {
		args.singleArgument ="/"+args.singleArgument+"/";
		return eval('('+args.singleArgument+'.test(v))');
	}],
	/*
	 * Usage: validate-ajax-$url
	 * Example: <input id='email' class='validate-ajax-http://localhost:8080/email.jsp'>
	 */
	...
	/*
	 * Usage: date-$dateFormat or date($dateFormat default is yyyy-MM-dd)
	 * Example: date-yyyy/MM/dd
	 */
	['date', function(v,elm,args,metadata) {
	...
	['one-selection', function(v,elm,args,metadata) {
	...
	//中国相关验证开始
	['id-number',function(v,elm,args,metadata) {
