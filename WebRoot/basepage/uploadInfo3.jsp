        ...
        <script type="text/javascript">	
			...
			function submitForm(){	
				if(valid.validate()==false)
					return;
				var url="<%=basePath%>saveFileInfo2.qact";
				//alert(url);
				$('#form1').ajaxSubmit({
				    type: "POST",
					url: url,
			...
			function saveData(){
				...
				if(fileCd != null && fileCd != ' ' && fileCd != 'null'){
					if(confirm("已上传会议材料，是否重新上传？")){
						submitForm();
					}
				}else{
					submitForm();
    ...
    <s:form method="post" name="form1" id="form1" enctype="multipart/form-data">  	
    	...
    	<div class="list1">
	        <table width="100%" border="0" cellspacing="0" cellpadding="0" id="tableContent">
	            ...
	            <td class="td_padding_right list1_td_style">&nbsp;&nbsp;选择材料：</th>
	    		...
	    		<table  width="100%" border="0" cellpadding="0" cellspacing="0" style="border: 0px solid;color: red;">
					<tr>
						<td align="center" style="border-right: 0px solid #D7D7D7;">
							<input type="button" value='保存' class="button_save" onclick="saveData()"/>
						</td>
					</tr>
				</table>
			</div>
