    ...
    <script type="text/javascript">
		...
		function SetStyle1(cls) {
		  ...
		  if(t.tagName=="LI")
			...
			{				
				...
				document.getElementById('mainTR').style.height=(cHeight-24-80-31)+"px";
				var tabh=0;
				if(document.getElementById('ul').style.display != "none"  )
					tabh=30;

				document.getElementById('content').style.height=(cHeight-24-80-31-26-tabh)+"px";
				document.getElementById('fraMain').style.height=(cHeight-24-80-31-26-tabh-2)+"px";
			}
		...
		<dl id="idBox" class="lightbox">
			...
			<br/>
				...
				<s:iterator id="org" value="%{orgList}" status="st">
					<s:if test="#st.index==0"> 
					    <input type="radio" name="orgItem" id="org${st.index}" value='<s:property value="#org.id"/>' checked />
					</s:if>
					<s:else>
						<input type="radio" name="orgItem" id="org${st.index}" value='<s:property value="#org.id"/>'  />
					</s:else>
					<label for="org${st.index}" id="orglab<s:property value="#org.id"/>">
			...
			<br/>
			<center>
				<input type="button" value=" 确 定 " onclick="orgSelect();" id="idBoxCancel" />
				<br/>
				<br/>
			</center>
			</dd>
		</dl>    
<table border="0" cellspacing="0" cellpadding="0"  style="width:100%;margin: 0px;" id="mainTable">
  <tr style="width:100%;height:24px;">
  	<td background="pageframe/images/home/topbj.gif">
			<table width="100%" height="100%" border="0"  cellpadding="0" cellspacing="0">
				<tr style="color:#777">
					<td nowrap="nowrap" style="padding-right: 10px;padding-left: 10px;" align="right" width="100%" >
						
						<a title="设置用户信息" id="logout" href="UpdateCurrentUserInfoView.qact" target="_blank" >
						<b>${userName}</b>
						<%-- 
						<security:authentication property="name" />
						<security:authentication property="principal.useOrgId" />
						--%>
						</a>欢迎您！
						<s:if test="orgName != null ">
						当前所属的组织是<span id="curOrg" style="font-weight: bold;">${orgName}</span>
						</s:if>
						<s:else>
							<span id="curOrg" style="font-weight: bold;"></span>
						</s:else>

						<div style="display:inline;" id="time"> </div>

					<s:if test="orgList.size >1">

						<a title="切换当前用户登录的组织" id="logout" href="javascript:showOrgSelect(); " target="_top">[切换组织]</a>						
					
 					</s:if>						

						<a title="注销当前登录用户" id="logout" href="j_spring_security_logout" target="_top">[注销登录]</a>						
					</td>				
			  	</tr>	
	  	  </table>  		
  	</td>
  ...
  <tr style="width:100%;height:80px;">
    <td height="80px"  width="100%" >
		<table style="height:80px;width:100%;background-image: images/home/1pixel.gif" border="0"  cellpadding="0" cellspacing="0" >
            <tr>
                <td width=510  height=80 background="pageframe/images/home/public-head1.gif"></td>
                <td id="region1_menu" valign="middle" align="right" nowrap background="pageframe/images/home/workhead1.gif" id=tdImgHead>
                &nbsp;</td>
            </tr>
	    </table>
    </td>
  ...
  <tr>
    <td height="31" bgcolor="#0033FF">
  		<table width="100%" border="0" cellpadding="0" cellspacing="0" class="nav_bj">
			<tr>
				<td width="20"><span id="menuId" style="display: none"></span></td>
  ...
  <tr id="mainTR">
    <td style="width:100%;" align="left" valign="top" bgcolor="#e6e6e6">
  		<table cellspacing=0 cellpadding=0 align=center border=0 style="WIDTH: 100%; HEIGHT: 100%">
			<tr>
				...
				<td align="center" width="8" valign="middle" id="sideBarTD" style="BORDER-RIGHT: #8c8c8c 1px solid" background=pageframe/images/pillarBG.gif>
					<img src="pageframe/images/togglemenulf.gif" border=0 id=imgMenu style="BORDER-LEFT: #8c8c8c 1px solid; CURSOR: hand" onclick="hideTreePage();"/>
				</td>	
					...
					<td>
				        <div class='tab' id="tab">			
				            <div id="ul" style="display:none">
				             ...
				             <div  id="content">
				                <iframe id="fraMain" class="content" src="" frameborder="0" width="100%"  scrolling="auto"></iframe>
				            </div>
			       		</div>    			
					</td>
					</tr>
